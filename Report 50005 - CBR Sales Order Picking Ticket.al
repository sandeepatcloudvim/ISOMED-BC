report 50005 "Sales Order Picking TicketCBR"
{
    // CBR_SS : 25/04/2018 : Added code to display Lot No. , Qty and Expiration Date.
    DefaultLayout = RDLC;
    RDLCLayout = './SalesOrderPickingTicketCBR.rdl';
    ...
    dataset
    {
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            column(ShipAcctNo; "Sales Header"."CBR Shipper AcctNo")
            {
            }
            ...
            column(Created_UserID; "Sales Header"."CBR Created By UserID")
            {
            }
            column(Created_UserName; "Sales Header"."CBR Created By User Name")
            {
            }
            column(ShiptoContact_CBR; "Sales Header"."Ship-to Contact")
            {
            }
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    dataitem(RoundLoop; "Integer")
                    {
                        ...
                        column(LotNo_1_Value; LotNo[1])
                        {
                        }
                        column(LotNo_2_Value; LotNo[2])
                        {
                        }
                        column(LotNo_3_Value; LotNo[3])
                        {
                        }
                        column(LotNo_4_Value; LotNo[4])
                        {
                        }
                        column(LotNo_5_Value; LotNo[5])
                        {
                        }
                        column(ExpDate_1_Value; ExpDate[1])
                        {
                        }
                        column(ExpDate_2_Value; ExpDate[2])
                        {
                        }
                        column(ExpDate_3_Value; ExpDate[3])
                        {
                        }
                        column(ExpDate_4_Value; ExpDate[4])
                        {
                        }
                        column(ExpDate_5_Value; ExpDate[5])
                        {
                        }
                        column(Qty_1_Value; Qty[1])
                        {
                        }
                        column(Qty_2_Value; Qty[2])
                        {
                        }
                        column(Qty_3_Value; Qty[3])
                        {
                        }
                        column(Qty_4_Value; Qty[4])
                        {
                        }
                        column(Qty_5_Value; Qty[5])
                        {
                        }
                        ...
                        begin
                            ...
                            if ReservEntry.FindSet then
                                repeat
                                    ...
                                    Qty[i] := Abs(ReservEntry.Quantity);
                                    if ReservEntry."Expiration Date" <> 0D then
                                        ExpDate[i] := Format(ReservEntry."Expiration Date")
                                    else
                                        ExpDate[i] := GetExpirationDate(ReservEntry);
                            ...
                            if TracSpec.FindSet then
                                repeat
                                    i += 1;
                                    LotNo[i] := TracSpec."Lot No.";
                                    Qty[i] := Abs(TracSpec."Quantity (Base)" / TracSpec."Qty. per Unit of Measure");
                                    ExpDate[i] := Format(TracSpec."Expiration Date");
